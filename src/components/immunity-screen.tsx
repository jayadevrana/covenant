    ...
    try {
      ...
      if (!response.ok || !payload.review) {
        const message = typeof payload.error === "string"
          ? payload.error
          : payload.error?.message ?? "Correction compile failed";
    ...
    <main className="mx-auto max-w-7xl px-6 py-12 md:px-8 md:py-16">
      <header className="flex flex-col justify-between gap-6 md:flex-row md:items-end">
        <div>
          ...
          <p className="mt-4 max-w-3xl text-base leading-7 text-slate-500">Convert one human correction into a generated eval and candidate patch, preview the deterministic proof, then choose whether policy changes.</p>
        </div>
      ...
      {(error ?? loadError) ? <div role="alert" className="mt-8 rounded-xl border border-rose-300/25 bg-rose-300/10 p-4 text-sm text-rose-100">{error ?? loadError}</div> : null}
      ...
      {!loading && (!run || decisions.length === 0) ? (
        <section className="mt-10 rounded-2xl border border-dashed border-white/10 p-12 text-center">
          <p className="text-sm text-slate-500">Complete a run first so there is a decided action to correct.</p>
      ...
      {run && decisions.length > 0 ? (
        <div className="mt-10 grid gap-6 xl:grid-cols-[22rem_1fr]">
          <aside className="h-fit rounded-2xl border border-white/10 bg-white/[0.02] p-5 xl:sticky xl:top-24">
            <p className="font-mono text-[10px] font-semibold uppercase tracking-[0.18em] text-slate-600">Human correction</p>
            ...
            <p className="mt-4 text-xs leading-5 text-slate-600">Compiler output is a suggestion. Only deterministic replay verifies it, and only your approval activates it.</p>
          </aside>
            ...
            {review ? (
              <>
                <section className="rounded-2xl border border-violet-300/15 bg-violet-300/[0.025] p-5 md:p-6">
                  ...
                  <p className="mt-4 text-sm leading-6 text-slate-400">{review.compilation.explanation}</p>
                  <div className="mt-5 rounded-xl border border-emerald-300/15 bg-black/25 p-4">
                    ...
                    <pre className="mt-4 max-h-80 overflow-auto rounded-lg border border-white/6 bg-black/25 p-4 text-[10px] leading-5 text-slate-500">{JSON.stringify(review.compilation.candidate_patch.rule, null, 2)}</pre>
                  </div>
                </section>
...
function RegressionGrid({ before, after }: { before: EvalResult[]; after: EvalResult[] }) {
  ...
  return (
    <section className="overflow-hidden rounded-2xl border border-white/10">
      <div className="grid grid-cols-[1fr_5rem_5rem] bg-white/[0.035] px-5 py-3 font-mono text-[9px] uppercase tracking-wider text-slate-600"><span>Regression eval</span><span>Before</span><span>After</span></div>
...
function Empty({ text }: { text: string }) {
  return <div className="rounded-2xl border border-dashed border-white/10 p-12 text-center text-sm text-slate-600">{text}</div>;
}
