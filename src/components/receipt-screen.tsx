    ...
    <main className="mx-auto max-w-7xl px-6 py-12 md:px-8 md:py-16">
      <header className="flex flex-col justify-between gap-6 md:flex-row md:items-end">
        <div>
          ...
          <p className="mt-4 max-w-3xl text-base leading-7 text-slate-500">Read every run event, verify its SHA-256 link in this browser, and export the evidence as JSON.</p>
        </div>
      ...
      {!loading && !run ? (
        <section className="mt-10 rounded-2xl border border-dashed border-white/10 p-12 text-center">
          <p className="text-sm text-slate-500">No run is available yet.</p>
      ...
      {run ? (
        <>
          <section className="mt-10 overflow-hidden rounded-2xl border border-white/10 bg-white/[0.02]">
            <div className="grid gap-px bg-white/8 md:grid-cols-[1fr_12rem_12rem]">
              <div className="min-w-0 bg-[#090c11] p-5">
                <p className="font-mono text-[9px] uppercase tracking-wider text-slate-600">Chain head</p>
                <code className="mt-2 block truncate text-xs text-violet-200">{head}</code>
              </div>
              <div className="bg-[#090c11] p-5"><p className="font-mono text-[9px] uppercase tracking-wider text-slate-600">Events</p><p className="mt-2 text-sm">{run.receipts.length}</p></div>
              <div className="bg-[#090c11] p-5"><p className="font-mono text-[9px] uppercase tracking-wider text-slate-600">Run</p><p className="mt-2 truncate text-sm">{run.id}</p></div>
            </div>
            <div className="flex flex-wrap items-center justify-between gap-4 border-t border-white/8 p-5">
              <p className="max-w-2xl text-xs leading-5 text-slate-600">Tamper-evident within this export. These hashes are not cryptographic signatures and do not prove events outside the routed sandbox path.</p>
          ...
          <section className="mt-6 overflow-hidden rounded-2xl border border-white/10">
            <div className="grid grid-cols-[3rem_10rem_1fr] bg-white/[0.035] px-5 py-3 font-mono text-[9px] uppercase tracking-wider text-slate-600">
            ...
            <ol>
              {run.receipts.map((receipt) => (
                <li className="grid grid-cols-[3rem_10rem_1fr] border-t border-white/6 px-5 py-4 text-xs" key={receipt.payload_hash}>
                  <span className="font-mono text-slate-600">{String(receipt.seq).padStart(2, "0")}</span>
                  ...
                  <div className="min-w-0">
                    ...
                    <details className="mt-2"><summary className="cursor-pointer text-slate-600">Read event payload</summary><pre className="mt-2 max-h-64 overflow-auto rounded-lg bg-black/30 p-3 text-[10px] leading-5 text-slate-500">{JSON.stringify(receipt.payload, null, 2)}</pre></details>
                  </div>
                </li>
              ))}
            </ol>
...
function Empty() {
  return <div className="mt-10 rounded-2xl border border-dashed border-white/10 p-12 text-center text-sm text-slate-600">Loading the latest receipt chain…</div>;
}
